    ...
    for group in groups:
        ...
        for suffix in suffixs:
            ...
            for log_dir in dirs:
                ...
                stall_secs = std_info.stall_duration_sec
                yerror_dict[device].append(stall_secs)
        ...
        for device in yerror_dict:
            ...
            row = [group, device, mean, std]
            rows.append(row)

        columns = ["group", "device", "average", "standard err"]

        result_pd = pd.DataFrame(rows, columns=columns)
        result_pd.to_csv("../csv_results/fillrandom/stall.csv", sep="\t", index=False)
