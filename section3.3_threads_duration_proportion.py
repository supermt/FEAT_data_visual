...
def to_human_format(device_string):
    return device_string.replace("SSD", " SSD").replace("HDD", " HDD")
...
if __name__ == '__main__':
    ...
    log_dir_prefix = "Eurosys/pm_stall_proportion_thread/"
    ...
    for log_dir in dirs:
        ...
        data_row_head = [to_human_format(std_result.device), int(std_result.cpu_count.replace("CPU", ""))]
        ...
        export_row.append(hms_to_sec(std_result.L0_stall_duration))
        export_row.append(hms_to_sec(std_result.Pending_stall_duration))
        export_row.append(hms_to_sec(std_result.Mem_stall_duration))
    ...
    origin_export_df = pd.DataFrame(export_csv_list, columns=["device", "thread number",
                                                              "L0 Stall", "PendingBytes Stall", "MemtableStall",
                                                              "L0 duration", "Pending duration", "Mem duration",
                                                              "throughput", "Stall Duration"])
    print(origin_export_df)

    stall_type_distribution_pd = origin_export_df.sort_values(by=["device", "thread number"],
                                                              ignore_index=True)

    stall_type_distribution_pd.to_csv("csv_results/section3.3_stall_proportion_64MB.csv", index=False, sep="\t")

    # log_dir_prefix = "Eurosys/pm_stall_proportion_batch/"
    #
    # # origin_painting_df = pd.DataFrame()
    # export_csv_list = []
    #
    ...
    # print(export_csv_list)
    #
    # origin_export_df = pd.DataFrame(export_csv_list, columns=["device", "thread number", "batch size",
    #                                                           "L0 Stall", "PendingBytes Stall", "MemtableStall",
    #                                                           "L0 duration", "Pending duration", "Mem duration",
    #                                                           "throughput", "Stall Duration"])
    # print(origin_export_df)
    ...
    #                                                           ignore_index=True)
    #
    # stall_type_distribution_pd.to_csv("csv_results/section3.3_stall_proportion.csv", index=False, sep="\t")
