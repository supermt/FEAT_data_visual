...
if __name__ == '__main__':
    ...
    for group in group_results:
        ...
        for device in devices:
            ...
            axes[row_count, col_count].plot(group_results[group][device][2], "r--")
            row_count += 1
        col_count += 1

    fig.tight_layout()
    fig.subplots_adjust(bottom=0.125, left=0.15)
    fig.text(0.025, 0.15, "Physical Memory Usage (MB)", ha='center', rotation="vertical", font={"size": 16.5})
    fig.text(0.5, 0.005, "Elapsed Time (Sec)", ha='center', font={"size": 16.5})

    fig.savefig('fig_results/rss_comparison.png')
    fig.savefig('fig_results/rss_comparison.pdf')
