...
def plot_with_thread_num(thread_num, batch_size, start_time, end_time):
    ...
    mapping_of_thoughs_mmo = axes[0].twinx()
    mapping_of_thoughs_throughput = axes[0].twinx()
    mapping_of_thoughs_throughput.set_ylim(-300, 250)
    mapping_of_thoughs_throughput.set_yticks([])
    throughput_line, = mapping_of_thoughs_throughput.plot(report_df["secs_elapsed"], report_df["interval_qps"])
    mapping_of_thoughs_mem_size = axes[0]
    mem_size_line, = mapping_of_thoughs_mem_size.plot(report_df["secs_elapsed"], report_df["total_mem_size"], "r--")
    mapping_of_thoughs_mem_size.set_ylim(0, 300)
    mapping_of_thoughs_mem_size.set_yticks([])
    ...
    label_list = [throughput_line, mem_size_line, MMO_point]
    plot_labels = ["System Throughput (kOps/Sec)", "Memory Component Size (MB)", "Occurrence of MMO"]
    ...
    fig.show()
    fig.savefig('fig_results/MMO_detail_mappings.pdf')
...
if __name__ == '__main__':
    ...
    plot_with_thread_num(4, "64MB", 0, 300)
