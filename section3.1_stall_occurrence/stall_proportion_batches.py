...
if __name__ == '__main__':
    ...
    log_dir_prefix = "../Eurosys/pm_server_increasing_batch/"

    for log_suffix in ["1", "2"]:
        LOG_DIR = log_dir_prefix + log_suffix + "/"
        ...
        result_df = pd.DataFrame(result_list,
                                 columns=["device", "# of thread", "batch size", 'mo duration', "lo duration",
                                          "ro duration"])
        result_df.to_csv("csv_results/stall_duration_breakdown.csv", index=False, sep="\t")
    # result_list = []
